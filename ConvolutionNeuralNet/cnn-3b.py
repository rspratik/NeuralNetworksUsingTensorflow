...
3.Can take some time for execution.
'''
cwd = "./cnn/logs"
...
def model(X, w1, w2, w3, w_fc, w_o, p_keep_conv, p_keep_hidden, act):
    # Y  [None, 10])
    ...
    '''Convulution layer3 with doubled feature maps(128) wrt conv layer2'''
    # l3a shape=(?, 14, 14, 128) ; l3[None,14,14,128] ;  W3[3, 3, 64, 128]
    l3a = tf.nn.relu(tf.nn.conv2d(l2, w3, strides=[1, 1, 1, 1], padding='SAME'))
    # l3 shape=(?, 14, 14, 128)
    l3 = tf.nn.max_pool(l3a, ksize=[1, 1, 1, 1], strides=[1, 1, 1, 1], padding='SAME')
    l3 = tf.nn.dropout(l3, p_keep_conv)

    '''Dense layer'''
    # reshape l3[None,14,14,128] to l4[None, 14*14*128]  : reducing dimensions.
    l4 = tf.reshape(l3, [-1, w_fc.get_shape().as_list()[0]]) #l4[None, 14*14*128]
    l4 = tf.nn.dropout(l4, p_keep_conv)

    '''Output layer'''
    l5 = act(tf.matmul(l4, w_fc)) #l4[None, 14*14*128]  and  W_FC[128*14*14, 625]
    l5 = tf.nn.dropout(l5, p_keep_hidden) #l5[none, 625]

    pyx = tf.matmul(l5, w_o)  #l5[none, 625] w_o [625, 10]
    return pyx # [none,10]
...
w_fc = init_weights([128 * 14 * 14, 625]) # FC 128 * 14 * 14 inputs, 625 outputs
w_o = init_weights([625, 10])         # FC 625 inputs, 10 outputs (labels)
...
py_x = model(X, w1, w2, w3, w_fc, w_o, p_keep_conv, p_keep_hidden, act=tf.nn.leaky_relu)
